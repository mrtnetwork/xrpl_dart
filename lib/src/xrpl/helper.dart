...
/// from an XRPL server.
Future<int> getAccountSequence(XRPLRpc client, String address) async {
  /// Fetch account information for the specified address.
  final accountInfo = await client.getAccountInfo(
      XRPAddressUtilities.toClassicAddress(address),
...
Future<void> autoFill(
  ...
  if (setupAccountSequence) {
    final int sequence = await getAccountSequence(client, transaction.account);
