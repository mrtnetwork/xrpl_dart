...
void _test() {
  test('Payment', () {
    ...
    final transaction = SubmittableTransaction.fromBlob(blob);
    expect(transaction.toBlob(), blob);
    expect(transaction.transactionType, SubmittableTransactionType.payment);
    expect(transaction.memos[0].memoData, memoData);
    expect((transaction as Payment).amount.value, BigInt.from(12000000));
  });
  test('Payment_2', () {
    const blob =
    ...
    final fromXrpl = SubmittableTransaction.fromBlob(transaction.toBlob());
  ...
  test('MintNFT', () {
    ...
    expect(transaction.transactionType, SubmittableTransactionType.nftokenMint);
  ...
  test('SignerListSet', () {
    ...
    expect(
        transaction.transactionType, SubmittableTransactionType.signerListSet);
    expect(transaction.signer?.signingPubKey,
  ...
  test('AccountSet', () {
    ...
    expect(transaction.toBlob(), blob);
    expect(transaction.account, 'rPQdZ1gtxLCZidafWTRDerhUeZAsZAvM29');
    expect(transaction.transactionType, SubmittableTransactionType.accountSet);
    expect(transaction.signer?.signingPubKey,
        '0372A211BC750CBC51D1D29FB3D1615FD508D714F44BDDCC475029037346DFE12D');
